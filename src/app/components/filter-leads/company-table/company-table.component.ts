...
export class CompanyTableComponent implements OnInit {
  ...
  availableFilters = [
    ...
    { id: 'ownership', name: 'نوع الملكية', type: 'select', options: [], isDynamic: true },
    { id: 'companyStage', name: 'مرحلة الشركة', type: 'select', options: [], isDynamic: true },
    { id: 'size', name: 'الحجم', type: 'select', options: [], isDynamic: true },
    { id: 'industry', name: 'الصناعة', type: 'select', options: [], isDynamic: true },
    { id: 'companyName', name: 'اسم الشركة', type: 'select', options: [], isDynamic: true },
    { id: 'country', name: 'الدولة', type: 'location', options: [], isDynamic: true },
    { id: 'city', name: 'المدينة', type: 'location', options: [], isDynamic: true }
  ];
  ...
  sizeOptions = ['صغيرة (1-10)', 'متوسطة صغيرة (11-50)', 'متوسطة (51-200)', 'كبيرة (201-1000)', 'كبيرة جداً (1000+)'];
  ...
  private applyFilters(data: CompanyData[], filters: any): CompanyData[] {
    return data.filter(item => {
      ...
      if (filters.country && !item.location?.toLowerCase().includes(filters.country.toLowerCase())) return false;
  ...
  getActiveFiltersCount(): number {
    ...
    // Debug logging
    console.log('Company Filter Count Debug:', {
      selectedFilters: this.selectedFilters.length,
      showCountryDropdown: this.showCountryDropdown,
      showCityDropdown: this.showCityDropdown,
      showJobTitleDropdown: this.showJobTitleDropdown,
      showJobLevelDropdown: this.showJobLevelDropdown,
      showIndustryDropdown: this.showIndustryDropdown,
      showComapnySizeDropdown: this.showComapnySizeDropdown,
      totalCount: count
    });
      ...
      if (filter.value) {
        switch (filter.id) {
          case 'companyName':
            ...
            const selectedCompany = this.allCompanyDropDown.find(company => company.id == filter.value);
            if (selectedCompany) {
          ...
          case 'industry':
            ...
            const selectedIndustry = this.industries.find(industry => industry.id == filter.value);
            if (selectedIndustry) {
          ...
          case 'size':
            ...
            const selectedSize = this.comapnySize.find(size => size.id == filter.value);
            if (selectedSize) {
              filterParams.companySize = selectedSize.sizeName || selectedSize.name;
            } else {
          ...
          case 'companyStage':
            ...
            const selectedStage = this.companyStage.find(stage => stage.id == filter.value);
            if (selectedStage) {
          ...
          case 'ownership':
            ...
            const selectedOwnership = this.allOwnerShip.find(ownership => ownership.id == filter.value);
    ...
    this.personalService.GetIndustry().subscribe({
      next: (res) => {
        if (res.data) {
          ...
          const industryFilter = this.availableFilters.find(f => f.id === 'industry');
          if (industryFilter) {
    ...
    this.companyService.GetAllCompanyDropDown().subscribe({
      next: (res) => {
        if (res.data) {
          ...
          const companyNameFilter = this.availableFilters.find(f => f.id === 'companyName');
  ...
  getCompanyStage() {
    this.companyService.GetCompanyStage().subscribe({
      next: (res) => {
        ...
        const companyStageFilter = this.availableFilters.find(f => f.id === 'companyStage');
  ...
  getAllOwnerShip() {
    this.companyService.GetAllOwnerShip().subscribe({
      next: (res) => {
        ...
        const ownershipFilter = this.availableFilters.find(f => f.id === 'ownership');
        if (ownershipFilter) {
